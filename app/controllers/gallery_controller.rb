class GalleryController < AdminController
	...
	def index
	  #@stores = Store.all
	   @optics_gallery = Store.paginate(:page => params[:page])
	end
  ...
  def checkout
		...
		flash[:notice]="Thank You for using Techno Optics. The oreder details are sent to your mail"
		session[:cart_id]=nil
  ...
  def search
    @optics_gallery = Store.find_by_sql ["Select * from stores WHERE product_name like ? or brand like ? or color like ? or description like ?","%"+params[:search],"%"+params[:search],"%"+params[:search],"%"+params[:search]]	  
  end
end
